...
TextEntity::TextEntity(float x, float y, float scale, std::string message, int fontSize, SDL_Color color, TTF_Font* font, RenderWindow* renderWindow)
{
    this->size = fontSize;
    this->color = color;
    this->font = font;
    this->rw = renderWindow;
...
void TextEntity::updateText(std::string message)
{
    ...
    SDL_Surface* textSurf = TTF_RenderText_Solid(font, message.c_str(), color);
    tex = rw->createFontTexture(textSurf);
    currentFrame = { 0, 0, textSurf->w, textSurf->h };
    SDL_FreeSurface(textSurf);
}
